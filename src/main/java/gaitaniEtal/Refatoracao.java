...
public class Refatoracao {
	...
	public void lerProjeto(String caminho) {
		try {
			...
			if (files != null && files.length > 0) {
				this.nullObject = new NullObject(caminho);// para só instanciar caso ache algo no diretório
				setIndiceClasse(0);
					...
					if (arquivo.isDirectory()) {
						...
						this.lerProjeto(caminho + "/" + arquivo.getName());
							...
							if (!l.erro) {
								...
								setIndiceClasse(getIndiceClasse() + 1);
							}
	...
	public void setNullObject(NullObject nullObject) {
		this.nullObject = nullObject;
	}

	public int getIndiceClasse() {
		return indiceClasse;
	}

	public void setIndiceClasse(int indiceClasse) {
